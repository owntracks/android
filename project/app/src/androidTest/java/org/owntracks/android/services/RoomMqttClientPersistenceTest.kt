...
class RoomMqttClientPersistenceTest {
    ...
    fun mqttClientPersistenceReadWriteWithNullPayloadTest() {
        ...
        RoomMqttClientPersistence(context, true).apply { open(null, null) }.use {
            val key = "testKey"
    ...
    fun mqttClientPersistenceRemoveTest() {
        ...
        RoomMqttClientPersistence(context, true).apply { open(null, null) }.use {
            ...
            it.put(key1, MqttPersistentData(key1, header1, 0, header1.size, payload1, 0, payload1.size))
            val key2 = "testKey2"
            val header2 = "header2".toByteArray()
            val payload2 = "payload2".toByteArray()
            it.put(key2, MqttPersistentData(key2, header2, 0, header2.size, payload1, 0, payload2.size))
