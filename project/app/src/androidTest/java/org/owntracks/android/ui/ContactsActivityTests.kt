...
class ContactsActivityTests :
    ...
    fun contactAppearsWhenBrokerAddsALocationMessage() {
        ...
        assertRecyclerViewItemCount(R.id.contactsRecyclerView, 1)
        assertDisplayedAtPosition(R.id.contactsRecyclerView, 0, R.id.name, "aa")
    }
    ...
    fun multipleContactsDisplayedInTimeStampOrder() {
        ...
        val random = Random(1)
        val timesAndNames = IntRange(0, random.nextInt(20)).map {
            TimeAndName(
        ...
        timesAndNames.sortedBy { it.instant }.reversed().forEachIndexed { index, s ->
    ...
    @Test
    fun contactPresentIfPublishedWithMQTTRetainFlag() {
        MessageLocation()
            .apply {
                latitude = 51.0
                longitude = 0.0
            ...
            .run {
                sendFromBroker(broker, retain = true)
            }
        setupTestActivity()
        sleep(1000)
        assertNotDisplayed(R.id.placeholder)
        ...
        assertRecyclerViewItemCount(R.id.contactsRecyclerView, 1)
        assertDisplayedAtPosition(R.id.contactsRecyclerView, 0, R.id.name, "aa")
        assertDisplayedAtPosition(R.id.contactsRecyclerView, 0, R.id.location, "51.0000, 0.0000")
    }
}
