...
inline fun IdlingResource?.use(timeout: Duration = 15.seconds, block: () -> Unit) {
  ...
  try {
    this?.run {
      Timber.tag("ARSE")
          .i("Registering idling resource ${this.name}@${this.hashCode()} for $timeout")
      val time = measureTime {
        IdlingRegistry.getInstance().register(this)
        Espresso.onIdle()
      }
      Timber.tag("ARSE").i("${this.name}@${this.hashCode()} is now idle after $time")
    }
    Timber.tag("ARSE")
        .i(
    ...
    this?.run {
      Timber.tag("ARSE").i("Finally unregistering idling resource ${this.name}@${this.hashCode()}")
      IdlingRegistry.getInstance().unregister(this)
    }
  }
}
