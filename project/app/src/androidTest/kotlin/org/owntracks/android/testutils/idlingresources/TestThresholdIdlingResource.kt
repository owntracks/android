...
class TestThresholdIdlingResource(
  ...
  init {
    if (enableLog) {
      Timber.tag("ARSE").d("Creating $name@${hashCode()}")
    }
  }
  ...
  override fun registerIdleTransitionCallback(resourceCallback: ResourceCallback) {
    if (enableLog) {
      Timber.tag("ARSE").d("Registering idle transition callback for $name@${this.hashCode()}")
    }
  ...
  override fun set(value: Int) {
    if (enableLog) {
      Timber.tag("ARSE").d("Setting $name value to $value threshold is $threshold")
    }
  ...
  override var threshold: Int = 0
    set(value) = run {
      if (enableLog) {
        Timber.tag("ARSE")
            .d("Setting $name threshold to $value current value is ${this.value.get()}")
      }
