...
data class LogEntry(
    ...
    private val priorityChar = when (priority) {
        Log.ASSERT -> "A"
        Log.ERROR -> "E"
        Log.WARN -> "W"
        Log.INFO -> "I"
    ...
    private val longDateFormat = SimpleDateFormat(
    ...
    /**
     * Returns the string indexes of the boundaries of the priority and tag components, so that we can highlight those
     *
     ...
     * @return
     */
    fun toExportedString(): String = "${longDateFormat.format(time)} $priorityChar [$threadName] $tag: $message"
}
