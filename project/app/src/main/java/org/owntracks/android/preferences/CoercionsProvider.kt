...
import org.owntracks.android.preferences.Preferences.Companion.EXPERIMENTAL_FEATURE_ALLOW_SMALL_KEEPALIVE
...
class CoercionsProviderImpl : CoercionsProvider {
    /**
     * Coerces a preference into an allowable range. Sometimes this coercion depends on the value of other preferences
     * so we need a way of getting those
     *
    ...
    override fun <T> getCoercion(property: KProperty<*>, value: T, preferences: Preferences): T {
        return when (property) {
            ...
            Preferences::keepalive -> {
                (value as Int)
                    .coerceAtLeast(
                        if (preferences.experimentalFeatures.contains(EXPERIMENTAL_FEATURE_ALLOW_SMALL_KEEPALIVE)) {
                            1
