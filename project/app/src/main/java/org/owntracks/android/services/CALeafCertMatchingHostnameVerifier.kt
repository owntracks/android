 ...
 */
class CALeafCertMatchingHostnameVerifier(caCertificate: Certificate) : HostnameVerifier {
    private val caFingerprint: ByteArray = MessageDigest.getInstance("SHA-1")
    ...
    override fun verify(hostname: String?, session: SSLSession?): Boolean {
        ...
        return if (caFingerprint.contentEquals(fingerprint)) {
            Timber.i(
                "CA Fingerprint matches server leaf cert: %s. Skipping hostname verification",
