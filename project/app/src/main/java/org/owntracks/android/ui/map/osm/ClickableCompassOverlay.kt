...
class ClickableCompassOverlay(
    ...
    /**
     * Tests to see if the given coordinates that are absolute to the display lie in a rectangle
     * with relative coordinates to the containing [MapView]
     *
    ...
    private fun hitTest(rect: Rect, mapView: MapView, x: Float, y: Float): Boolean =
        IntArray(2)
            .apply(mapView::getLocationOnScreen)
            .let { coords ->
                rect.apply {
